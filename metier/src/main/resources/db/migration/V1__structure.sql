...
create table client (id bigint generated by default as identity (start with 1), date_creation timestamp, nom varchar(255), adresse_id bigint, primary key (id));
...
create table consomation (id bigint not null, date_creation date, prix_paye double, quantite integer, produit_id bigint, reservation_id bigint, primary key (id));
...
alter table consomation add constraint FK9ffj492mae8twsn4qdl08nvw5 foreign key (produit_id) references produit;
alter table consomation add constraint FKmr2pegluby9t2uko43ktg2e79 foreign key (reservation_id) references reservation;
