...
insert into LIBELLE (clef, libelle, langue) values ('RESERVATION_DEJA_EXISTANTE', 'A reservation already exists those days', 'en');
insert into LIBELLE (clef, libelle, langue) values ('SUPPRESSION_IMPOSSIBLE_OBJETS_LIES', 'Can not delete this object because of linked ''{{0}}''.', 'en');
insert into LIBELLE (clef, libelle, langue) values ('TRANSITION_ETAT_IMPOSSIBLE', 'Can not change state from ''{{0}}'' to ''{{1}}''.', 'en');
insert into LIBELLE (clef, libelle, langue) values ('RESERVATION_DATES_INCOHERENTES', 'Invalid start and end dates.', 'en');
-- Page de réservation
